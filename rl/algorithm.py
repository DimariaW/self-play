...
class IMPALA(ActorCriticBase):
    ...
    def learn(self):
        ...
        bootstrap_mask = 1. - batch["only_bootstrap"]  # 0表示被mask掉

        value_info, action_logit = self.model(obs)  # shape: B*T, B*T*act_dim
        ...
        clipped_c = torch.clamp(rho, 0, 1)  # clip_c_threshold := 1)  c shape: B*T
        ...
        if self.upgo_key is not None:
            for key in self.upgo_key:
                ...
                actor_loss_local = torch.sum(-action_log_prob * clipped_rho * upgo_adv)
                actor_loss += actor_loss_local
